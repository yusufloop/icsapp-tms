// components/booking/Step3Form.tsx
...
export default function Step3Form({ data, onDataChange }: Step3FormProps) {
  ...
  useEffect(() => {
    const search = async () => {
      if (!searchQuery.trim()) {
        if (drivers.length > 0) loadDrivers(); // Reload only if there's a need
        return;
      }
      try {
  ...
  const handleBookmark = () => {
    ...
    Alert.alert(isBookmarked ? "Template Removed" : "Template Saved", isBookmarked ? "Configuration removed from templates" : "Configuration from Steps 1 & 2 saved as a template");
  };

  const getDriverAvatar = (name: string) => driverAvatars[name.length % driverAvatars.length];

  if (loading) {
    return <View className="items-center py-12"><ActivityIndicator size="large" color="#0A84FF" /><Text className="text-text-secondary mt-4">Loading drivers...</Text></View>;
  }
  ...
  return (
    <>
      <View className="mb-6">
        <View className="rounded-lg bg-white border border-gray-300 flex-row items-center px-4 py-3 mb-4">
        ...
        <View className="flex-row items-center justify-between">
          <View className="flex-row space-x-3">
            <TouchableOpacity className="bg-white border border-gray-300 rounded-lg px-4 py-2 flex-row items-center"><MaterialIcons name="sort" size={18} color="#1C1C1E" /><Text className="text-sm font-medium text-text-primary ml-2">Sort</Text></TouchableOpacity>
            <TouchableOpacity className="bg-white border border-gray-300 rounded-lg px-4 py-2 flex-row items-center relative"><MaterialIcons name="filter-list" size={18} color="#1C1C1E" /><Text className="text-sm font-medium text-text-primary ml-2">Filter</Text><View className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full items-center justify-center"><Text className="text-xs font-bold text-white">2</Text></View></TouchableOpacity>
          </View>
          <TouchableOpacity onPress={handleBookmark} className="p-2"><MaterialIcons name={isBookmarked ? "bookmark" : "bookmark-border"} size={24} color={isBookmarked ? "#0A84FF" : "#8A8A8E"} /></TouchableOpacity>
        </View>
      </View>
      ...
      <View>
        {transformedDrivers.length === 0 ? (
          <View className="items-center py-12">
            ...
            <Text className="text-text-secondary mt-4 text-center">{searchQuery ? 'No drivers found matching your search' : 'No drivers available'}</Text>
          </View>
        ) : (
          transformedDrivers.map((driver) => (
            <TouchableOpacity
              ...
              className={`bg-white border rounded-lg p-4 mb-3 flex-row items-center ${data.selectedDriverId === driver.id ? "border-primary bg-blue-50" : "border-gray-300"}`}
            >
              <TouchableOpacity onPress={() => onDataChange('selectedDriverId', driver.id)} className="mr-4">
                <View className={`w-6 h-6 rounded-full border-2 items-center justify-center ${data.selectedDriverId === driver.id ? "border-primary bg-primary" : "border-gray-400"}`}>
                  {data.selectedDriverId === driver.id && <View className="w-3 h-3 rounded-full bg-white" />}
                </View>
              </TouchableOpacity>
              <View className="w-12 h-12 rounded-full bg-gray-200 items-center justify-center mr-4"><Text className="text-2xl">{getDriverAvatar(driver.name)}</Text></View>
            ...
            {modalDriver && (
                <View>
                    <View className="px-6 py-6 border-b border-gray-200 relative">
                        <TouchableOpacity onPress={() => setShowDriverModal(false)} className="absolute top-4 right-4 z-10"><MaterialIcons name="close" size={24} color="#1C1C1E" /></TouchableOpacity>
                    ...
                    {modalDriver.status.toLowerCase() === 'available' ? (
                        <View className="px-6 py-8 items-center">
                            ...
                            <Text className="text-xl font-bold text-green-700">Driver Available</Text>
                            <Text className="text-text-secondary text-center">This driver has no active booking.</Text>
                        </View>
                    ) : (
